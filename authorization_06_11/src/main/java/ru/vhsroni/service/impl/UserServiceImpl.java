...
public class UserServiceImpl implements UserService {
    ...
    public AuthResponse signIn(SignInRequest request) {
        String attr = request.getSignInAttr();
        UserEntity user;
        if (findUserByEmail(attr) != null) {
            if (attr == null || attr.isBlank())
                return response(1, "Empty email", null);
            if (!AuthUtils.checkEmail(attr))
                return response(4, "Invalid email", null);
            user = findUserByEmail(attr);
        } else if (findUserByNickname(attr) != null) {
            if (attr == null || attr.isBlank())
                return response(3, "Empty nickname", null);
            user = findUserByNickname(attr);
        } else return response(8, "Email/nickname not found", null);
    ...
    public AuthResponse signInByToken(String token) {
        try {
            ...
            if (user == null) {
                log.warn("Сouldn't sign in with a token. Response status 10, description `Token not found`");
                return response(10, "Token not found", null);
            }
            if (user.getToken() == null || user.getTokenUsage() < 1) {
                log.warn("Сouldn't sign in with a token. Response status 11, description `Token expired`");
                return response(11, "Token expired", null);
            }

            jdbcTemplate.update(PropertyReader.getSqlQuery("SQL_UPDATE_USER"), user.getTokenUsage() - 1,
                    user.getId());
            log.info("Sign in by token ended successfully. TokenUsage = {}", user.getTokenUsage());
            return response(0, "OK", findUserById(user.getId()));
        } catch (EmptyResultDataAccessException e) {
            log.warn("Сouldn't sign in with a token - EmptyResultDataAccessException");
            return response(10, "Token not found", null);
        }
    }
